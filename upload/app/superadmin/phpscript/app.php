...
foreach ($scan as $name) {
	if ($name && $name != '.' && $name != '..' && filetype($tpath . $name) == 'dir') {
		if (file_exists($tpath . $name . '/config.xml') && !file_exists($tpath . $name . '/install.json') && file_exists($tpath . $name . '/setting.html')) {
...
$navhtml = template('superadmin:app-nav', TRUE);
		...
		if ($name && $name != '.' && $name != '..' && filetype($tpath . $name) == 'dir') {
			if (file_exists($tpath . $name . '/config.xml')) {
				...
				if (file_exists($tpath . $name . '/install.json')) {
					$var = 'newinstallhtml';
					$ahtml = "
					<a class='pk-text-success pk-hover-underline' href='index.php?c=app&a=superadmin:index&s=app&t={$name}&ml=install'>安装</a>
					";
				} else {
					$var = 'oldinstallhtml';
					$ahtml = "
					<a class='pk-text-danger pk-hover-underline' href='index.php?c=app&a=superadmin:index&s=app&t={$name}&ml=uninstall'>卸载</a>
	...
	if ($_G['GET']['ML'] == 'install') {
		...
		if (file_exists($fpath)) {
			...
			if ($jsondata) {
				...
				rename($fpath, "{$_G['SYSTEM']['PATH']}/app/{$_G['GET']['T']}/uninstall.json");
				$alert = '恭喜，安装完成！' . mysql_error();
			} else {
				$alert = '安装数据解析出错！';
			}
		} else {
		...
		if (file_exists($fpath)) {
			...
			if ($jsondata) {
				...
				rename($fpath, "{$_G['SYSTEM']['PATH']}/app/{$_G['GET']['T']}/install.json");
				$alert = '恭喜，卸载完成！';
			} else {
				$alert = '卸载数据解析出错！';
			}

...
foreach ($scan as $name) {
	if ($name && $name != '.' && $name != '..' && filetype($tpath . $name) == 'dir') {
		if (file_exists($tpath . $name . '/setting.html') && file_exists($tpath . $name . '/config.xml') && !file_exists($tpath . $name . '/install.json')) {
...
$navhtml = template('superadmin:template-nav', TRUE);
		...
		if ($name && $name != '.' && $name != '..' && filetype($tpath . $name) == 'dir') {
			if (file_exists($tpath . $name . '/config.xml')) {
				...
				if (file_exists($tpath . $name . '/install.json')) {
					$var = 'newinstallhtml';
					$ahtml = "
					<a class='pk-text-success pk-hover-underline' href='index.php?c=app&a=superadmin:index&s=template&t={$name}&ml=install'>安装</a>
					";
				} else {
					...
					$ahtml .= "<a class='pk-text-danger pk-hover-underline' href='index.php?c=app&a=superadmin:index&s=template&t={$name}&ml=uninstall'>卸载</a>
	...
	if ($_G['GET']['ML'] == 'install') {
		...
		if (file_exists($fpath)) {
			...
			if ($jsondata) {
				...
				rename($fpath, "{$_G['SYSTEM']['PATH']}/template/{$_G['GET']['T']}/uninstall.json");
				$alert = '恭喜，安装完成！';
			} else {
				$alert = '安装数据解析出错！';
			}
		...
		if (file_exists($fpath)) {
			...
			if ($jsondata) {
				...
				rename($fpath, "{$_G['SYSTEM']['PATH']}/template/{$_G['GET']['T']}/install.json");
				$alert = '恭喜，卸载完成！';
			} else {
				$alert = '卸载数据解析出错！';
			}

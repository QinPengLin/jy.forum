...
class Data {
	...
	public function getData($field = NULL, $str = NULL) {
		if ($field === NULL) {
			return FALSE;
		}
		if (Cnum($field, FALSE)) {
	...
	public function getDatas($pos = 0, $rnum = 10, $field = NULL, $str = NULL) {
		$limit = " limit {$pos},{$rnum}";
		if ($field)
			$sql = ' ' . $this -> getSql($field, $str);
		if (!$rnum)
			$limit = '';
		$query = mysql_query("select * from `{$this -> mysql_prefix}{$this -> table}`{$sql}{$limit}");
		if ($query) {
			$array = array();
			$index = 0;

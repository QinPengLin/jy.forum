...
if ($_G['USER']['ID'] == 1 || InArray(getUserQX(), 'admin') || $_G['TEMP']['BKADMIN'] || ($_G['USER']['ID'] == $replyuserdata['id'] && $_G['USER']['ID'])) {
	...
	if (InArray(getUserQX(), 'delreply')) {
		$_G['TEMP']['READADMINLINK'] .= '&nbsp;<a href="javascript:" onclick="pkalert(&quot;确认删除该回复？&quot;,&quot;提示&quot;,&quot;window.open(\'index.php?c=delete&table=reply&id=' . $replydata['id'] . '&chkcsrfval=' . $_G['CHKCSRFVAL'] . '\',\'pk-di\');$(\'#replydivbox-' . $replydata['id'] . '\').remove()&quot;)">删除</a>';
	}
}
